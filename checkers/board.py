...
class Board:
    ...
    def draw_squares(self, screen):
        ...
        for row in range(ROWS):
            for col in range(row % 2, COLS, 2):
                pygame.draw.rect(screen, TEAL, (row*SQUARE_SIZE, col*SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))
            ...
            for cols in range(COLS):
                if cols % 2 == (row + 1) % 2:
                    if row < 3:
                        ...
                        self.board[row].append(Piece(row, cols, TEAL))
                    else:
    ...
    def get_valid_moves(self, piece):
        ...
        if piece.color == TEAL or piece.king:
